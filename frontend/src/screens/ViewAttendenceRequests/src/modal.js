...
import { selecUpdatedAttendance } from "../../../states/reducers/slices/backend/UserSlice";

import Edit from "./Edit";
import { deleteAttendanceRequest, updateAttendanceRequest } from "../../../api/attendence";

export default function ViewAttendanceRequest({ selectedId }) {
  const [showModal, setShowModal] = useState(false);
  const [edit, setEdit] = useState(false);
  const attendance = useSelector(selecUpdatedAttendance)

  const handleDelete = () => {
    console.log("selectedId:", selectedId);
    deleteAttendanceRequest(selectedId)
  }

  const handleSubmit = () => {
    updateAttendanceRequest(attendance, selectedId);
      setShowModal(false);
      setEdit(false);
  }
