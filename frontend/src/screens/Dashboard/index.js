...
import { closeAll, selectNavState, selectWidth, setIsMenuOpen } from '../../states/reducers/slices/frontend/Navbar';
...
const Dashboard = () => {
    ...
    return (
        <div className={`flex`} onClick={() => dispatcher(closeAll())}>
            <Navbar />
            <Topbar />
            {/* <button onClick={() => logout(dispatcher, navigation)}>log out</button> */}
            <div className="text-black p-7 px-4 relative top-[5rem] w-[calc(100%-269px)] duration-300" style={{left: width, width:(open)?'':'100vw'}}>
                <Outlet />
            </div>
        </div>
    )
