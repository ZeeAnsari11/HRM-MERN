...
//   // console.log("======desiginationsbbbbb===", desiginations);
//   const [selectedPeople, setSelectedPeople] = useState([]);

//   // useEffect(() => {
//   //   setSelectedPeople(formData.designations || []);
//   // }, []);
//   useEffect(() => {
        ...
        <div className="max-h-40 overflow-y-auto">
          {selectedPeople.map((id, index) => {
            ...
            return designation ? (
              <span
                key={index}
                className="inline-flex items-center bg-gray-200 text-gray-700 rounded-full text-sm font-medium px-2 py-1 mr-1 mb-1"
              >
                {designation.title}
                <span
                  className="ml-1 text-gray-500 hover:text-red-500 focus:outline-none cursor-pointer"
                  onClick={() => handleRemoveDesignation(id)}
                >
                  <X size={16} />
                </span>
              </span>
