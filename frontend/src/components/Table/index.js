...
function GlobalFilter({
  ...
  return (
    <div className="flex gap-x-2 items-baseline rounded-lg">
      <div className="relative h-full mobile:w-full">
          ...
          <input type="search"
              ...
              placeholder={`${count} records...`} className="block h-full w-full px-4 py-2 pl-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-black-500 focus:outline "
            />
      </div>
    </div>
  )
...
function Table({ columns, data, element }) {
  ...
  if (data.length > 0)
  {
    return (
      <>
        <div className="flex justify-between mobile:flex-col mobile:space-y-2">
            <GlobalFilter
  ...
  else return <NotFound element={element}/>
}
