...
export const createShortLeaveType = (req, res, next) => {
    try {
        if (!req.body.organization || !req.body.name) throw 'Invalid Body'
...
export const updateShortLeaveTypeById = (req, res, next) => {
    try {
        if (req.body.organization) throw 'Invalid Body'
        if (req.body.name) {
            ShortLeaveTypeModel.findById(req.params.id)
                .then((shortLeaveType) => {
                    if (!shortLeaveType) throw `No such short leave type ${req.params.id}`
