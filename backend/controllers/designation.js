...
export const createDesignation = (req, res, next) => {
    UserModel.find({ _id: req.body.createdBy, organization: req.body.organization})
        .then((response) => {
            ...
            req.body.designationKey = (req.body.organization + req.body.shortForm).toUpperCase();
...
export const updateDesignationById = (req, res, next) => {
    try {
        if (req.body.organization || req.body.createdBy || req.body.designationKey) throw "You can not update organization Creater of Designation"
        if (req.body.shortForm) {
            DesignationModel.findById(req.params.id)
                .then((designatin) => {
                    req.body.designationKey =( designatin.organization + req.body.shortForm).toUpperCase();
